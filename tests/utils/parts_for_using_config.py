...
def create_param_grid_from_config(config):
    ...
    if 'model' in config:
        for model_name, model_params in config['model'].items():
            ...
            if model_name == 'outliers':
                param_grid['outlier_handler__strategy'] = model_params['strategies']
                param_grid['outlier_handler__z_thresh'] = model_params['thresholds']
    
    # Create encoder hyperparams combinations
    features_with_alternatives = []
    for feature, feature_config in config['encoding']['features'].items():
        if 'alternatives' in feature_config:
            features_with_alternatives.append({
                'feature': feature,
                'options': [feature_config['strategy']] + feature_config['alternatives']
            })
    
    # Generate all combinations for features with alternatives
    import itertools
    
    if features_with_alternatives:
        encoder_param_combinations = []
        feature_names = [f['feature'] for f in features_with_alternatives]
        feature_options = [f['options'] for f in features_with_alternatives]
        
        for combination in itertools.product(*feature_options):
            hyperparams_dict = {feature_names[i]: combination[i] for i in range(len(feature_names))}
            encoder_param_combinations.append(hyperparams_dict)
        
        param_grid['encoder__hyperparams'] = encoder_param_combinations
